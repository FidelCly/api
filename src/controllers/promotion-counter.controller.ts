...
import { PromotionCounterRepository, PromotionRepository, UserRepository } from "../repositories";
...
export class PromotionCounterController {
	...
	static oneByClient = async (req: Request, res: Response) => {
		...
		try {
			const promotion = await PromotionCounterRepository.findOneUser(shopId, userId, promotionId);
			res.status(200).send(promotion);
		} catch (error) {
			res.status(404).send({ message: "Promotion not found" });
		}
	...
	static create = async (req: Request, res: Response) => {
		...
		try {
			...
			if (!user) res.status(400).send({ message: `user with id '${userId}' not found` });
	...
	static update = async (req: Request, res: Response) => {
		const payload: IPromotionCounterUpdatePayload = Object.assign({}, req.body as IPromotionCounterUpdatePayload);

		if (Object.keys(payload).length === 0) res.status(400).send({ message: "Validation failed" });
	...
	static delete = async (req: Request, res: Response) => {
		...
		try {
			...
			res.status(400).send({ message: "Error while deleting promotion counter" });
		}
	};
}
